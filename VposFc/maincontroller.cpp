...
void MainController::insertItem(const QString& name, const QString& price, const QString &oriprice, const QString &promtype)
{
    long long priceNum = getMoneyValue(price);
    emit addItem(name, 1, priceNum, price, oriprice, promtype);
}
...
int MainController::getAppIndex()
{
    ...
    appnames << "canteenvng" << "ffbanhmique" << "ffcomngon" << "ffcomtamcali" << "ffcomnieu" << "fftapicocup" << "fftokyobowl" << "ffvietthai" << "ffphongoon" << "ffcool" << "ffcobonla" << "ffdorayaki" << "maybanhangtudong" << "mqshops" << "vpos";
...
void MainController::setAppName(const QString &appName, bool initSocket)
{
    ...
    emit updateTitle(QString("%1 - %2").arg(appTitle).arg(machineName));
