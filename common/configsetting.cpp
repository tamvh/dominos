...
void ConfigSetting::Load()
{
    ...
    centerService   = "";
    printBep        = sets.value(QString("option/printBep"),        false).toBool();
...
void ConfigSetting::Save()
{
    ...
    sets.sync();
    sets.setValue(QString("option/sysidletime"), idleTimer);
    sets.sync();
...
void ConfigSetting::setIdletimer(const int timer)
{
    if( idleTimer != timer ) {
        idleTimer = timer*1000;
...
void ConfigSetting::setIdCouponPercent(float percent) {
